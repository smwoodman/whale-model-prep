...
### Plot biweekly maps
for(i in names(x.14d.sf)[108:385]) {
  print(i)
  i.date <- as.Date(substr(i, 11, 20), format = "%Y_%m_%d")
  i.date.end <- i.date + days(14)
  png(paste0("../raimbow-local/Plots/Plots_KAF/Plots_KAF_Barca_continuous/", i,".png"), width = 4, height = 6, units = 'in', res = 450)
  
  plot(x.14d.sf[i], main = NA, axes = TRUE, 
       border = NA, breaks = c(seq(0, 0.080, by = 0.0001), 0.085), #c(seq(0, 0.075, by = 0.005), 0.085), 
  ...
  text(-127.7, 32.7, labels = txt.lab, pos = 4)
