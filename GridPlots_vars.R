# Function to plot model input variables
...
gridplots_vars <- function(files.toplot, plot.folder, var.name, 
  ...
  stopifnot(
    ...
    inherits(plot.folder, "character")
  )
  
  if (is.null(col.pal)) {
    n.b <- ifelse(is.null(col.breaks), 6, length(col.breaks))
    col.pal <- brewer.pal(n.b - 1, "YlGnBu")
  }
  ...
  for (i in files.toplot) {
    ...
    png(paste0(plot.folder, file.tosave), units = "in", width = 4, height = 6, res = 300)
