...
# For 10-km 49729 segments, takes 3-4 minutes
# 
Crop.Area = TRUE
...
# Specify folder for needed fils with grid midpoints and static predictors 
...
fixeddata$effort <- 1 #rep(1,nrow(fixeddata))
species.data <- fixeddata[, c("pixel", "mlat", "mlon")]
...
# 2-2.3s per file -> ~80 minutes for 2374 
...
for (i in week.start:week.end) {        # specify weeks above
  ...
  vardata$yearCoVar <- as.numeric(gridYR)
...
#
rm(wc8yrs, wc8yrs.nona, wc8yrs.nona.no0)
...
rm(alldens,avgdens.1991,avgdens.1993,avgdens.1996,avgdens.2001,avgdens.2005,avgdens.2008, 
...
species.data <- cbind(species.data[,1:3], n.Dens=ndens, Avg.Dens=avgdens, 
...
species.data.Surf <- cbind(species.data[ ,1:11])
species.data.Surf<- data.frame(species.data.Surf)
names(species.data.Surf) <- c("pixel","mlat","mlon","n.Dens","Avg.Dens","STD.Dens", 
                              "L90STD.dens","U90STD.dens","SE.Dens","L90SE.dens","U90SE.dens")
write.table(species.data.Surf,file="Bp_AveDens_ROMSdaily_XY_YR_MAXDF_15.csv",sep=",",row.names=FALSE)
